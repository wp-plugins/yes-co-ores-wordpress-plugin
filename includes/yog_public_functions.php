<?php
  ...
  */
  ...
  function yog_contact_widget_shortcode($atts)
  {
    if (empty($atts['id']))
    {
      return 'No id provided for yes-co contact form.';
    }

    $widgetNr = $atts['id'];

    $widgetId = 'yogcontactformwidget-' . $widgetNr;
    ...
    if (!empty($wp_registered_widgets[$widgetId]))
    {
      ...
      if (!empty($widget['callback']) && !empty($widget['callback'][0]))
      {
        ...
        $widgetInstance = $widget['callback'][0];

        $options = get_option($widgetInstance->option_name);

        $instance = $options[$widgetNr];
        $class = get_class( $widgetInstance );
        $widgets_map = yog_widget_shortcode_get_widgets_map();
	    $_original_widget_position = $widgets_map[$widgetId];
        ...
        $html = $widgetInstance->widget($params[0], $instance);
      }
    }
   ...
   * @return array
   */
  function yog_widget_shortcode_get_widgets_map() {
