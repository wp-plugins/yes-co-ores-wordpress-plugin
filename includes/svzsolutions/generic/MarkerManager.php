<?php
  ...
  class SVZ_Solutions_Generic_Marker_Manager
  {
    ...
    public function setMarkerTitleTemplate($template, $zoomLevel)
    {
      ...
      if ($zoomLevel > 20)
        throw new Exception(__METHOD__ . '; Invalid $zoomLevel, max is 20');
    ...
    public function toArray()
    {
      ...
      foreach ($markers as $key => $marker)
      {
        if ($marker instanceof SVZ_Solutions_Generic_Marker_List || $marker instanceof SVZ_Solutions_Generic_Marker_Cluster)
        {
          ...
          {
            $clusterMarker            = array();
            $clusterMarker['type']    = $marker->getTypeName();
            $clusterMarker['label']   = $numSubMarkers;
            $clusterMarker['title']   = $marker->getTitle() . ': ' . $numSubMarkers;
            $clusterMarker['count']   = $numSubMarkers;

            if ($marker instanceof SVZ_Solutions_Generic_Marker_List)
            {
              $clusterMarker['entityIds']   = $marker->getEntityIds();
              $clusterMarker['dataLoadUrl'] = $marker->getListDataLoadUrl();
            }

            $geocodeCenter            = $marker->getBounds()->getGeocodeCenter();

            $clusterMarker['geoLat']  = $geocodeCenter->getLatitude();
            $clusterMarker['geoLng']  = $geocodeCenter->getLongitude();

            // $clusterMarker['geoLat']  = $marker->getGeocode()->getLatitude();
            // $clusterMarker['geoLng']  = $marker->getGeocode()->getLongitude();

            if ($marker->hasAddress())
              $clusterMarker['address'] = $marker->getAddress()->toArray();
    ...
    private function clusterByDistance($markers, $distance)
    {
      ...
      while ($loopMarkers->valid())
      {
        ...
        if (in_array($viewPort->getZoomLevel(), array(17, 18, 19, 20)))
          $clusterMarker                = new SVZ_Solutions_Generic_Marker_List($this->listDataLoadUrl);
        else
          $clusterMarker                = new SVZ_Solutions_Generic_Marker_Cluster();
