  ...
  */
  function yog_retrieveSpecs($specs, $postId = null)
  {
    ...
    foreach ($specs as $spec)
    {
      ...
      {
        // Transmorf value
        if (!empty($typeMapping[$postMetaName]))
        {
          if (!empty($typeMapping[$postMetaName]['type']))
          {
            switch ($typeMapping[$postMetaName]['type'])
            {
              case 'oppervlakte':
          ...
          if (!empty($typeMapping[$postMetaName]['title']))
            $spec = $typeMapping[$postMetaName]['title'];
  ...
  function yog_retrievePrices($priceTypeClass = 'priceType', $priceConditionClass = 'priceCondition', $postId = null)
  {
    ...
    foreach ($priceFields as $field)
    {
      ...
      if (!empty($price))
      {
        ...
        $values[] = (empty($priceType) ? '' : '<span class="' . $priceTypeClass . '">' . $priceType . ': </span> ') . $price . (empty($priceCondition) ? '' : ' <span class="' . $priceConditionClass . '">' . $priceCondition . '</span>');
      }
    }
...
function yog_retrieveLinks($postId = null)
{
  ...
  */
  ...
  if (!is_admin())
    add_action('wp_head', 'yog_includeDojo');
  ...
  function yog_retrievePhotoSlider($largeImageSize = 'large', $thumbnailSize = 'thumbnail', $scrollable = false)
  {
    ...
    if (!empty($largeImages) && count($largeImages) > 0 && !empty($largeImages[0][0]))
    {
      $html = '<div id="imageactionsholder" class="clearfix">
                 <div class="mainimage">
                   <img id="bigImage" alt="" src="' . $largeImages[0][0] . '" />
                 </div>
               </div>';
               
      if (!empty($thumbnails) && count($thumbnails) > 1 && count($thumbnails) == count($largeImages))
      {
        $html .= '<div id="imgsliderholder" style="display: none;">';
        if ($scrollable === true)
          $html .= '<div class="left"><a title="Vorige foto" onclick="return false;" onmouseout="stopScroll();" onmouseover="ScrollLeft(\'imgslider\');" href="#">&nbsp;</a></div>';
        ...
        if ($scrollable === true)
          $html .= '<div class="right"><a title="Volgende foto" onclick="return false;" onmouseout="stopScroll();" onmouseover="ScrollRight(\'imgslider\');" href="#">&nbsp;</a></div>';
        
        $html .= '</div>';
        
        foreach ($thumbnails as $key => $thumbnail)
        {
          $thumbs[] = $largeImages[$key][0] . '<>' . $thumbnail[0];
        }
        
        $html .=  '<script type="text/javascript">
                    dojo.addOnLoad(function()
                      {
                        addThumbnails(\'' . json_encode($thumbs) . '\', ' . ($scrollable ? 'true' : 'false') . ');
                      });
                  </script>';
                  
        // Let's not discriminate people without javascript :-)
        $html .= '<noscript>';
        ...
        foreach ($thumbnails as $key => $thumbnail)
        {
          ...
          $html .= '<a href="' . $largeImage[0] . '"><img alt="" src="' . $thumbnail[0] . '" /></a>';
        }
